...
if img_gray is None or img_color is None:
    raise FileNotFoundError("Make sure 'flower.png' is in the same directory.")
...
for levels in [2, 4, 8, 16, 32, 64, 128]:
    ...
    cv2.imwrite(f"outputs/intensity_{levels}.png", reduced)
...
for k in [3, 10, 20]:
    ...
    cv2.imwrite(f"outputs/blur_{k}x{k}.png", blurred)
...
for angle in [45, 90]:
    ...
    cv2.imwrite(f"outputs/rotated_{angle}.png", rotated)
...
for b in [3, 5, 7]:
    ...
    cv2.imwrite(f"outputs/blockwise_avg_{b}x{b}.png", reduced_img)

print("All tasks completed.The outputs of the task are store in the output folder.")
